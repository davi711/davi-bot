...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Davi\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=553188514445:+55 31988514445\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '!'
blocked = []
limitawal = '999999999'
cr = 'TAMO VERIFICADO VLW ZUCK'
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `OIá @${num.split('@')[0]}\nBem vindo ao ${mdata.subject}\n\nESPERO QUE GOSTE DO GRUPO E LEIA AS REGRAS️`
				...
				teks = ` @${num.split('@')[0]} SAIU PORQUE É HETERO`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
	...
	if (messagesC.includes("fdp")){
			client.updatePresence(from, Presence.composing)
			reply("filha da puta? minha pica que te cutuca")
	}
	
		if (messagesC.includes("corno")){
			client.updatePresence(from, Presence.composing)
			reply("só estou te imitando boizão")
	}
 ...
 Presence.composing)
			...
			switch(command) {
				...
				case 'testing':
					...
					if (args.length < 1) return reply('Cadê o texto, hum')
					reply(mess.wait)
					anu = await fetchJson(`https://zeksapi.herokuapp.com/api/watercolour?text1=${gbl3}&text2=${gbl4}&apikey=xptn?352`, {method: 'get'})
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, image, {quoted: mek})
					break
				case 'snowrite':
...
// recursos anime, random @Causs domina vida
          ...
          reply('Espere..')
			    ...
			    case 'criador':
                    ...
                    client.sendMessage(from, 'CTT DO MEU DONO SALVA O CTT DELE E FALA PRA ELE TE SALVAR TBM',MessageType.text, { quoted: mek} )
                    break
                case 'fitnah':	
				case 'fake':          
                    if (!isGroup) return reply(mess.only.group)
				...
				case 'loli1':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'ENTÃO @FBI, O ${pushname2} QUER VER LOLI OQUE A GENTE FAZ ?'})
					break
				case 'hentai':
					memein = await kagApi.memeindo()
				...
				case 'canal':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️Canal do CRX:\nhttps://youtube.com/channel/UCpgkoI5IKsZoA2-v01Jt5OA'})
					break
				case 'nsfwloli1':
					memein = await kagApi.memeindo()
				...
				case 'reislin':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: ' IIHHH ALA O CARA VAI BATER UMA PRA IMAGEM QUE BOT MANDA'})
					break
				case 'boanoite':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://imgur.com/gallery/4HeRfuO`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'bom noite fml ❤️'})
					break
				case 'bomdia':
					memein = await kagApi.memeindo()
		...
		break
				...
				case 'banir':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'ALVO REMOVIDO COM SUSSESO CHEFE! :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
